...
1. Apps Script 編輯器 → 選函式 `setRotateSecret` → 先把參數填進去執行一次：

   ```js
   function 啟用() { setRotateSecret('你自己想的一串密語'); }
   ```

   密語存在**指令碼屬性**，不在 `Code.gs` 原始碼裡（這個 repo 是公開的）。
...
停用：執行 `clearRotateSecret()`，並清空 `/qr` 的密語欄。
