...
- 簽到成功後畫面會顯示名冊上的姓名（「王小明，已記錄簽到時間…」），學生自己就能確認有沒有打錯學號
...
| 學生打的 | 系統收到 |
| --- | --- |
| `b123456789` | `B123456789` |
| `  B123456789  ` | `B123456789` |
| `B-147-610-001` | `B123456789` |
| `Ｂ１４７６１０００１`（全形） | `B123456789` |
| `B　147610001`（全形空白） | `B123456789` |
| `b123456789@example.edu.tw` | `B123456789` |

輸入框會邊打邊清乾淨，同時保住游標位置（不然刪字會亂跳）。

查不到學號時，後端會找「幾乎確定是打錯」的候選並提示：

- **形近字**：`B12345678O`（數字 0 打成英文 O）→ 提示 `B123456780`
- **漏打開頭字母**：`123456781` → 提示 `B123456781`

畫面會出現「改用 B123456780 送出」的按鈕，**但不會自動套用** —— 簽到成別人比多按一下糟得多。
